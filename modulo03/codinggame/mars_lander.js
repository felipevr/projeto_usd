...
while (true) {
    ...
    const angle = parseInt(inputs[5]); // the rotation angle in degrees (-90 to 90).
    const power = parseInt(inputs[6]); // the thrust power (0 to 4).

    // Write an action using console.log()
    // To debug: console.error('Debug messages...');
    console.error('MP: ', X, Y, hSpeed, vSpeed, fuel, angle, power);
    


    let output = fireItUp(X, Y, hSpeed, vSpeed, fuel, angle, power);
...
function fireItUp(X, Y, hSpeed, vSpeed, fuel, angle, power) {
